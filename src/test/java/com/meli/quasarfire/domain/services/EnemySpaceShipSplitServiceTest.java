...
public class EnemySpaceShipSplitServiceTest {
	...
	public void setUp() {
		...
		Mockito.doNothing().when(this.satelliteRepository).saveSatellite(satelliteBuilder.build());
		Mockito.doReturn(satelliteList).when(this.satelliteRepository).getSatellites();
	}
	
	@Test
	void GivenSatellitesWhenSaveSatelliteThenGetSatelliteStored() {
